...
def todo_list(request):
    ...
    todos = Todo.objects.filter(complete=False) # false 일때만 조회 
    # todos = Todo.objects.all()  # 전체 조회할 때

    return render(request, 'todo/todo_list.html', {'todos': todos})
# p84 Todo 상세 조회 뷰 만들기
...
def todo_post(request):
    if request.method == "POST":
        ...
        form = TodoForm()
    return render(request, 'todo/todo_post.html', {'form': form}) #입력한 화면에 가서 연결
...
def todo_edit(request, pk):
    ...
    #######################################################################
# p92 완료한 Todo 목록 뷰 함수
#######################################################################
# 완료된 Todo 목록을 보여주는 뷰  (todo_list.html에서 "완료한 TODO 목록" 클릭시 실행)
def done_list(request):
    # DB에서 complete=True 인 Todo 객체만 조회
    dones = Todo.objects.filter(complete=True) #완료 리스트 
    # 조회된 객체들을 'done_list.html' 템플릿에 전달
    return render(request, 'todo/done_list.html', {'dones': dones}) #완료된것만 페이지에 보여줌 
